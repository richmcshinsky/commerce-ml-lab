...
class TestTrainTestSplit:
    def test_test_window_has_correct_days(self, synthetic_m5_long: pd.DataFrame) -> None:
        train, test = train_test_split(synthetic_m5_long, test_days=28)
        assert test["date"].nunique() == 28

    def test_no_overlap_between_splits(self, synthetic_m5_long: pd.DataFrame) -> None:
        train, test = train_test_split(synthetic_m5_long, test_days=28)
        train_dates = set(train["date"].dt.date)
        test_dates = set(test["date"].dt.date)
        assert len(train_dates & test_dates) == 0

    def test_train_before_test(self, synthetic_m5_long: pd.DataFrame) -> None:
        train, test = train_test_split(synthetic_m5_long, test_days=28)
        assert train["date"].max() < test["date"].min()

    def test_total_rows_preserved(self, synthetic_m5_long: pd.DataFrame) -> None:
        train, test = train_test_split(synthetic_m5_long, test_days=28)
        assert len(train) + len(test) == len(synthetic_m5_long)

    def test_all_skus_in_test(self, synthetic_m5_long: pd.DataFrame) -> None:
        train, test = train_test_split(synthetic_m5_long, test_days=28)
        assert set(test["id"]) == set(synthetic_m5_long["id"])


class TestZeroRate:
    def test_returns_float_in_range(self, synthetic_m5_long: pd.DataFrame) -> None:
...
class TestAggregateByDate:
    def test_returns_one_row_per_date(self, synthetic_m5_long: pd.DataFrame) -> None:
        agg = aggregate_by_date(synthetic_m5_long)
        n_dates = synthetic_m5_long["date"].nunique()
        assert len(agg) == n_dates

    def test_group_aggregation_has_correct_rows(self, synthetic_m5_long: pd.DataFrame) -> None:
    ...
    def test_total_sales_preserved(self, synthetic_m5_long: pd.DataFrame) -> None:
        agg = aggregate_by_date(synthetic_m5_long)
        assert agg["sales"].sum() == synthetic_m5_long["sales"].sum()


class TestMakeSeriesPivot:
    def test_pivot_shape(self, synthetic_m5_long: pd.DataFrame) -> None:
        wide = make_series_pivot(synthetic_m5_long)
        n_dates = synthetic_m5_long["date"].nunique()
        n_series = synthetic_m5_long["id"].nunique()
        assert wide.shape == (n_dates, n_series)

    def test_pivot_has_datetime_index(self, synthetic_m5_long: pd.DataFrame) -> None:
        wide = make_series_pivot(synthetic_m5_long)
        assert isinstance(wide.index, pd.DatetimeIndex)

    def test_pivot_no_nan(self, synthetic_m5_long: pd.DataFrame) -> None:
        wide = make_series_pivot(synthetic_m5_long)
        assert wide.isna().sum().sum() == 0


class TestGetActiveItems:
    def test_filters_sparse_items(self, synthetic_m5_long: pd.DataFrame) -> None:
    ...
    def test_high_threshold_returns_fewer_items(self, synthetic_m5_long: pd.DataFrame) -> None:
    ...
    def test_returns_list_of_strings(self, synthetic_m5_long: pd.DataFrame) -> None:
